...
const StarRating = ({ rating }) => {
  return (
    <div className="star-rating">
      {[1, 2, 3, 4, 5].map((s) => (
        <FiStar key={s} className={s <= Math.round(rating) ? 'star-filled' : 'star-empty'} />
      ))}
    </div>
  );
};
...
const ProductCard = ({ product, onWishlistToggle, inWishlist }) => {
  ...
  const imageUrl = product.images?.[0] || 'https://images.unsplash.com/photo-1567016432779-094069958ea5?w=400';

  return (
    <Link to={`/products/${product._id}`} className="product-card fade-in">
      <div className="product-card-image">
        <img src={imageUrl} alt={product.name} loading="lazy" />
        {product.quantity === 0 && <div className="out-of-stock-badge">Out of Stock</div>}
      ...
      <div className="product-card-info">
        ...
        <div className="product-footer">
          ...
          {product.isHandmade && <span className="handmade-tag">✦ Handmade</span>}
        </div>
      </div>
    </Link>
  );
